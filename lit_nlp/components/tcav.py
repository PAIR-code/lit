...
RELATIVE_TCAV_SPLITS = [3, 5, 7, 10, 15]
MIN_SPLIT_SIZE = 3
...
class TCAVConfig(object):
  ...
  negative_set_ids: Optional[List[str]] = []
...
class TCAV(lit_components.Interpreter):
  ...
  def _run_relative_tcav(self, grad_layer, emb_layer, grad_class_key,
    ...
    if (len(positive_outputs) < MIN_SPLIT_SIZE * 2 or
        len(negative_outputs) < MIN_SPLIT_SIZE * 2):
      splits = [min_length]
    ...
    for split in splits:
      ...
      # Many CAVS are trained and checked for statistical testing to calculate
      # the p-value. The values of the CAV with the highest accuracy is
      # returned.
      results.append({
          'result': concept_results[index],
          'p_val': p_val,
