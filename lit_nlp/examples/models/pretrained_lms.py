...
class BertMLM(lit_model.BatchedModel):
  ...
  def predict_minibatch(self, inputs):
    ...
    out: transformers.modeling_tf_outputs.TFMaskedLMOutput = self.model(
        encoded_input
    )
    batched_outputs = {
...
class GPT2LanguageModel(lit_model.BatchedModel):
  ...
  def _pred(self, encoded_inputs):
    ...
    """
    out: transformers.modeling_tf_outputs.TFCausalLMOutputWithPast = self.model(
