...
const NO_T_TESTING_WARNING =
    `Did not run t-testing (requires at least 6 positive and 6 negative samples)`;
...
export class TCAVModule extends LitModule {
  ...
  @observable private readonly toggledSlices = new Set<string>();
  ...
  get slicePairs(): Array<[string, string|null]> {
    ...
    const negativeSlices: string[] = Array.from(this.toggledSlices.values());
  ...
  renderCollapseBar(
      ...
      columnName: string, selectSet: Set<string>, toggleColumnName: string = '',
      toggledSet: Set<string>|null = null) {
    ...
    const data = items.map((item) => {
      ...
      if (toggledSet != null) {
        const toggleChanged = (e: Event, item: string) => {
          const toggle = e.target as HTMLInputElement;
          if (toggle.checked) {
            toggledSet.add(item);
          } else {
            toggledSet.delete(item);
          }
        };
        row.push(
            // clang-format off
            html`<mwc-switch id='compare-switch'
                  ?checked=${toggledSet.has(item)}
                  @change='${(e: Event) => {toggleChanged(e, item);}}'>
                </mwc-switch>`
            // clang-format on
        );
      }
            ...
            <div class="controls-buttons">
              <button id='clear-button' class='text-button'
                ...
                ?disabled=${this.selectedClasses.size === 0 &&
                  ...
                  this.toggledSlices.size === 0}>Clear</button>
              <button id='submit'
  ...
  private async runTCAV() {
    ...
    for (const res of results) {
      ...
      if (res['p_val'] != null && res['p_val'] > 0.05) {
        displayScore = '-';
